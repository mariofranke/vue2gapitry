<template>
  <v-app>
    <v-app-bar app elevation="4">
      ...
      <v-spacer></v-spacer>
      <div>
        <v-switch v-model="$vuetify.theme.dark" color="primary" hide-details></v-switch>
      </div>
      <v-btn
          id="login-button"
          class="mx-2"
      ...
      <v-menu v-if="userStore.isLoggedIn" offset-y>
        <template #activator="{ on }">
          ...
          >
            <v-avatar size="32px">
              <img :src="userStore.userData.picture">
            </v-avatar>
            {{ userStore.userData.name }}
          </v-btn>
        </template>
...
const showEvents = () => {
  ...
  client.callback = (resp) => {
    ...
    //getUserData(window.gapi.client.getToken().access_token)
...
/*const getUserData = () => {
  ...
  xhr.onload = function () {
    ...
    //this.setUserData(JSON.parse(xhr.responseText))
  };
  xhr.send();
